...
import { 
  MapPin, Plus, ExternalLink, Phone, 
  ArrowLeft, Map, Locate, Loader2 
} from 'lucide-react';
import { Link } from 'react-router-dom';
import { 
  calculateDistance, getCountryFlag, groupLocationsByCountry, 
  categoryIcons, FitnessLocation 
} from '@/lib/location';
import { 
  useLocationDiscovery, useLocationMutations, 
  initialFormData, type LocationFormData 
} from '@/hooks/locations';
import { 
  LocationCard, LocationSubmitDialog, LocationEditDialog, 
  LocationSearchDialog, LocationFilters 
...
const FitnessLocations = () => {
  ...
  const { data: locations, isLoading } = useQuery({
    queryKey: [
      ...
      selectedCategory, 
      searchQuery, 
      discovery.nearMeMode, 
      discovery.userLocation?.lat
    ],
    queryFn: async () => {
      let query = supabase
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    mutations.submitLocation.mutate(formData);
  };

  const handleEditSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingLocation) {
      mutations.updateLocation.mutate({ id: editingLocation.id, data: editFormData });
    }
  };
    ...
    <div className="min-h-screen bg-background p-4 md:p-6">
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <LocationSubmitDialog 
            open={isSubmitDialogOpen} 
            onOpenChange={setIsSubmitDialogOpen} 
            formData={formData} 
            setFormData={setFormData} 
            onSubmit={handleSubmit} 
            isPending={mutations.submitLocation.isPending} 
        ...
        <LocationFilters 
          ...
          nearMeMode={discovery.nearMeMode} 
          setNearMeMode={discovery.setNearMeMode} 
          discoverMode={discovery.discoverMode} 
          setDiscoverMode={discovery.setDiscoverMode} 
          locationLoading={discovery.locationLoading} 
          discoverLoading={discovery.discoverLoading} 
          locationName={discovery.locationName} 
          discoverCategory={discovery.discoverCategory} 
          setDiscoverCategory={discovery.setDiscoverCategory} 
          userLocation={discovery.userLocation} 
          onFindNearMe={() => discovery.handleUseGpsLocation(false)} 
          onChangeLocation={() => setIsLocationDialogOpen(true)} 
          onRefreshDiscover={() => {
            if (discovery.userLocation) {
              discovery.handleDiscoverAtLocation(
                discovery.userLocation.lat, 
                discovery.userLocation.lng
              );
            }
        ...
        {!discovery.discoverMode && (
          <DirectoryContent 
            ...
            isAdmin={isAdmin}
            onEdit={handleEdit}
            onDelete={(id) => mutations.deleteLocation.mutate(id)}
            onAddNew={() => setIsSubmitDialogOpen(true)}
            t={t}
          />
        )}
        ...
        <LocationEditDialog 
          ...
          isPending={mutations.updateLocation.isPending} 
          categories={categories} 
        />
        ...
        <LocationSearchDialog 
          ...
          setLocationInput={discovery.setLocationInput} 
          discoverCategory={discovery.discoverCategory} 
          setDiscoverCategory={discovery.setDiscoverCategory} 
          searchRadius={discovery.searchRadius} 
          setSearchRadius={discovery.setSearchRadius} 
          radiusUnit={discovery.radiusUnit} 
          setRadiusUnit={discovery.setRadiusUnit} 
          locationLoading={discovery.locationLoading} 
          geocodeLoading={discovery.geocodeLoading} 
          onUseGps={() => discovery.handleUseGpsLocation(true, () => setIsLocationDialogOpen(false))} 
          onSearch={() => discovery.handleLocationSearch(() => setIsLocationDialogOpen(false))} 
          categories={categories} 
        />
      </div>
    </div>
...
interface DiscoverModeContentProps {
  discovery: ReturnType<typeof useLocationDiscovery>;
...
const DiscoverModeContent = ({ 
  discovery, mutations, t, setIsLocationDialogOpen, setIsSubmitDialogOpen 
}: DiscoverModeContentProps) => {
  if (discovery.discoverLoading) {
    return (
  ...
  if (discovery.discoveredGyms.length > 0) {
    return (
      <>
        <p className="text-muted-foreground">
          {t('locations:discover_results', { count: discovery.discoveredGyms.length })}
        </p>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              >
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    ...
                    <Button 
                      size="sm" 
                      onClick={() => discovery.handleAddDiscoveredGym(gym, mutations.invalidateLocations)} 
                      disabled={discovery.addingGymId === gym.osm_id}
                    >
                      {discovery.addingGymId === gym.osm_id 
  ...
  return (
    <Card className="p-8 text-center">
      ...
      <Button 
        onClick={() => discovery.handleUseGpsLocation(false)} 
        disabled={discovery.locationLoading}
      >
        {discovery.locationLoading 
...
interface DirectoryContentProps {
  isLoading: boolean;
  locations: FitnessLocation[] | undefined;
  viewMode: 'list' | 'map';
  nearMeMode: boolean;
  isAdmin: boolean;
...
const DirectoryContent = ({ 
  isLoading, locations, viewMode, nearMeMode, isAdmin, onEdit, onDelete, onAddNew, t 
                ...
                <div key={country} className="space-y-4">
                  <div className="flex items-center gap-3 border-b border-border pb-2">
                    ...
                    <Badge variant="secondary" className="ml-auto">
                      {countryLocations.length} {countryLocations.length === 1 
                        ? t('locations:location') 
                        : t('locations:locations_count')
                      }
                    </Badge>
                  </div>
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
