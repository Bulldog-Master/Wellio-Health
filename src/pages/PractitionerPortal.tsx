...
import { ArrowLeft, Stethoscope, Users, Calendar, DollarSign, FileText, Clock, CheckCircle, XCircle, Loader2 } from 'lucide-react';
...
const PractitionerPortal = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: appData } = await supabase
        ...
        .select('*')
        .eq('user_id', user.id)
        .eq('professional_type', 'practitioner')
        .maybeSingle();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const applicationData = {
        user_id: user.id,
        professional_type: 'practitioner' as const,
  ...
  return (
    <SubscriptionGate feature="practitioner_portal">
      <div className="min-h-screen bg-background">
        <div className="container max-w-4xl py-8 px-4">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-3xl font-bold">{t('practitioner_portal')}</h1>
              <p className="text-muted-foreground">{t('practitioner_portal_desc')}</p>
            </div>
          </div>

          {application?.status === 'approved' ? (
            // Approved Practitioner Dashboard
              ...
              <TabsContent value="clients">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{clients.length} patients</CardDescription>
                  </CardHeader>
                  <CardContent>
                    {clients.length === 0 ? (
                      ...
                      <div className="space-y-4">
                        {clients.map(client => (
                          <div key={client.id} className="flex items-center justify-between p-4 border rounded-lg">
                            <div>
                              <p className="font-medium">{client.profiles?.full_name || 'Patient'}</p>
