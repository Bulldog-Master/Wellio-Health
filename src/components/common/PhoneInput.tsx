...
export const PhoneInput = ({ value, onChange, country = '', placeholder, className }: PhoneInputProps) => {
    ...
    if (country) {
      ...
      if (!found) {
        ...
        const matchingKey = Object.keys(countryCodes).find(
          key => key.toLowerCase() === lowerCountry || key.toLowerCase().includes(lowerCountry) || lowerCountry.includes(key.toLowerCase())
        );
        if (matchingKey) found = countryCodes[matchingKey];
      }
  ...
  const formatPhoneNumber = (input: string, format: string): string => {
    ...
    if (digitIndex < digits.length) result += digits.slice(digitIndex);
    return result;
  };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (countryCode && !newValue.startsWith(countryCode)) {
      ...
      const countryInfo = Object.entries(countryCodes).find(
        ([key]) => key.toLowerCase() === normalizedCountry.toLowerCase() || key.toLowerCase().includes(normalizedCountry.toLowerCase())
      )?.[1];
      if (countryInfo) {
