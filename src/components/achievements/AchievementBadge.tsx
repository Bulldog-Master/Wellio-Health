    ...
    <Dialog>
        ...
        <Card className={`cursor-pointer hover:shadow-lg transition-all ${isNew ? 'animate-pulse ring-2 ring-primary' : ''}`}>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              {isNew && <Badge variant="default" className="animate-pulse">New!</Badge>}
            </div>
            {nextLevel && (
              <div className="mt-3">
                ...
                <p className="text-xs text-muted-foreground mt-1">{value} / {nextLevel.value} to {nextLevel.label}</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>{unlockedAt && `Unlocked ${new Date(unlockedAt).toLocaleDateString()}`}</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="flex items-center justify-center p-6">
            <div className={`p-6 rounded-full bg-muted ${config.color}`}><Icon className="w-16 h-16" /></div>
          </div>
          <div className="text-center">
            <h3 className="text-2xl font-bold mb-2">{currentLevel.label}</h3>
            <p className="text-muted-foreground">You've achieved {value} in {config.title}</p>
          </div>
          <div className="space-y-2">
            <h4 className="font-semibold text-sm">All Levels</h4>
            {config.levels.map((level) => (
              <div key={level.value} className={`flex items-center justify-between p-2 rounded ${value >= level.value ? 'bg-primary/10' : 'bg-muted/50'}`}>
                <div className="flex items-center gap-2">
                  {value >= level.value ? <Star className="w-4 h-4 text-primary fill-current" /> : <Star className="w-4 h-4 text-muted-foreground" />}
                  <span className={value >= level.value ? 'font-semibold' : 'text-muted-foreground'}>{level.label}</span>
                </div>
