# Wellio Wellness APP Platform — Documentation Index

Quick reference to all project documentation for the Wellio Wellness APP Platform.

**Master Positioning:** Wellio is a privacy-first Wellness APP Platform for individuals, with optional support from trainers, coaches, and clinicians. Raw data stays on the user's device — only derived wellness insights can be shared, if the user chooses.

## 🔐 Security & Privacy

| Document | Description |
|----------|-------------|
| [SECURITY_AUDIT.md](./SECURITY_AUDIT.md) | Complete security assessment and RLS review |
| [SECURITY_FAQ.md](./SECURITY_FAQ.md) | Common security questions for external reviewers |
| [SECURITY_FIXES.md](./SECURITY_FIXES.md) | Security improvements and vulnerability remediation |
| [AI_PRIVACY.md](./AI_PRIVACY.md) | AI data handling, anonymization, and privacy policies |
| [MEDICAL_FILE_SECURITY.md](./MEDICAL_FILE_SECURITY.md) | Medical records encryption and HIPAA considerations |
| [ISO_COMPLIANCE.md](./ISO_COMPLIANCE.md) | ISO 27001/27701/27018 alignment |

## 🏗️ Architecture

| Document | Description |
|----------|-------------|
| [ARCHITECTURE.md](./ARCHITECTURE.md) | High-level system architecture |
| [DOMAIN_ARCHITECTURE.md](./DOMAIN_ARCHITECTURE.md) | Domain boundaries and module organization |
| [CODEBASE_ORGANIZATION.md](./CODEBASE_ORGANIZATION.md) | File structure and naming conventions |
| [REFACTORING_GUIDE.md](./REFACTORING_GUIDE.md) | Code refactoring patterns and best practices |

## 📣 Launch & Marketing

| Document | Description |
|----------|-------------|
| [BRAND_MESSAGING_GUIDE.md](./BRAND_MESSAGING_GUIDE.md) | Internal brand terminology and messaging |
| [MARKETING_POSITIONING.md](./MARKETING_POSITIONING.md) | Marketing copy and positioning |
| [PRESS_KIT.md](./PRESS_KIT.md) | Press release and media assets |
| [APP_STORE_DESCRIPTIONS.md](./APP_STORE_DESCRIPTIONS.md) | iOS/Android app store copy |
| [SOCIAL_CONTENT_PACK.md](./SOCIAL_CONTENT_PACK.md) | Ready-to-post social media content |
| [FOUNDER_VIDEO_SCRIPT_V2.md](./FOUNDER_VIDEO_SCRIPT_V2.md) | Founder video script (polished) |
| [PITCH_DECK_DESIGN_NOTES.md](./PITCH_DECK_DESIGN_NOTES.md) | Slide-by-slide deck specifications |
| [VISUAL_SYSTEM.md](./VISUAL_SYSTEM.md) | Colors, typography, iconography |
| [LANDING_PAGE_COPY.md](./LANDING_PAGE_COPY.md) | Website landing page structure |

## 🚀 Operations & Launch

| Document | Description |
|----------|-------------|
| [LAUNCH_CHECKLIST.md](./LAUNCH_CHECKLIST.md) | Pre-launch, launch day, and post-launch tasks |
| [TESTING_CHECKLIST.md](./TESTING_CHECKLIST.md) | Critical flow testing requirements |
| [BACKUP_RECOVERY.md](./BACKUP_RECOVERY.md) | Disaster recovery and backup procedures |
| [INCIDENT_RUNBOOK.md](./INCIDENT_RUNBOOK.md) | Production incident response procedures |
| [APP_STORE_PREP.md](./APP_STORE_PREP.md) | iOS/Android app store submission requirements |

## ⚡ Performance & UX

| Document | Description |
|----------|-------------|
| [PERFORMANCE_OPTIMIZATION.md](./PERFORMANCE_OPTIMIZATION.md) | Performance best practices |
| [LOADING_STATES.md](./LOADING_STATES.md) | Loading state patterns |
| [EMPTY_STATES.md](./EMPTY_STATES.md) | Empty state design patterns |
| [OPTIMISTIC_UPDATES.md](./OPTIMISTIC_UPDATES.md) | Optimistic UI update patterns |
| [OFFLINE_SUPPORT.md](./OFFLINE_SUPPORT.md) | PWA and offline capabilities |
| [ERROR_HANDLING.md](./ERROR_HANDLING.md) | Error handling patterns |
| [INPUT_VALIDATION.md](./INPUT_VALIDATION.md) | Form validation patterns |
| [RATE_LIMITING.md](./RATE_LIMITING.md) | Rate limiting implementation |

## 🌍 Internationalization

| Document | Description |
|----------|-------------|
| [TRANSLATION_GUIDE.md](./TRANSLATION_GUIDE.md) | Translation workflow and i18n patterns |
| [TRANSLATION_AUDIT_FINAL.md](./TRANSLATION_AUDIT_FINAL.md) | 23-language coverage audit |

## 💰 Monetization

| Document | Description |
|----------|-------------|
| [REWARDS_SYSTEM.md](./REWARDS_SYSTEM.md) | Points, rewards, and gamification |

## ♿ Accessibility

| Document | Description |
|----------|-------------|
| [ACCESSIBILITY.md](./ACCESSIBILITY.md) | WCAG compliance and accessibility patterns |

## 👥 User Guides

| Document | Description |
|----------|-------------|
| [USER_ACCESS_GUIDE.md](./USER_ACCESS_GUIDE.md) | User roles and permissions |
| [SECURITY_APPLICATION_REQUIREMENTS.md](./SECURITY_APPLICATION_REQUIREMENTS.md) | Security requirements for professional applications |

## 👥 Care Team & Professionals

| Document | Description |
|----------|-------------|
| [ACCESS_MODEL.md](./ACCESS_MODEL.md) | Who sees what — visibility matrix and consent rules |
| [PROFESSIONAL_ECONOMICS.md](./PROFESSIONAL_ECONOMICS.md) | Professional incentives, tiers, and revenue model |
| [CMIXX_INTEGRATION.md](./CMIXX_INTEGRATION.md) | xx.network cMixx metadata protection integration |
| [CARE_TEAM_PRO_TEST_PLAN.md](./CARE_TEAM_PRO_TEST_PLAN.md) | Smoke test checklist for Care Team features |

## 📊 Improvement Tracking

| Document | Description |
|----------|-------------|
| [IMPROVEMENTS_V2.md](./IMPROVEMENTS_V2.md) | Latest engineering improvements (performance, animations, i18n) |

---

## Quick Links

- **New to the project?** Start with [ARCHITECTURE.md](./ARCHITECTURE.md)
- **Security reviewer?** See [SECURITY_FAQ.md](./SECURITY_FAQ.md) and [SECURITY_AUDIT.md](./SECURITY_AUDIT.md)
- **AI/Privacy concerns?** See [AI_PRIVACY.md](./AI_PRIVACY.md)
- **Ready to launch?** See [LAUNCH_CHECKLIST.md](./LAUNCH_CHECKLIST.md)
