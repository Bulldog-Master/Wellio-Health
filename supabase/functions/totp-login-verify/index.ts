...
serve(async (req) => {
  console.log('[TOTP Login Verify] Request received:', req.method);
  ...
  try {
    ...
    console.log('[TOTP Login Verify] Auth header present:', !!authHeader);
    
    if (!authHeader) {
      console.error('[TOTP Login Verify] No authorization header');
      return new Response(
    ...
    console.log('[TOTP Login Verify] Verifying JWT token...');
    const { data: { user }, error: userError } = await supabaseAdmin.auth.getUser(token);
    
    if (userError || !user) {
      console.error('[TOTP Login Verify] Error getting user:', userError);
      return new Response(
    ...
    console.log('[TOTP Login Verify] User authenticated:', user.id);
    ...
    if (isValid) {
      console.log('[TOTP Login Verify] Login verification successful!');
      return new Response(
        JSON.stringify({ success: true, verified: true }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    } else {
      console.log('[TOTP Login Verify] Invalid token');
      return new Response(
    ...
    console.error('[TOTP Login Verify] Unexpected error:', error);
    return new Response(
